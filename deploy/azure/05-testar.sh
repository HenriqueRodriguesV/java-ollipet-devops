...
echo ""
echo "Para conferir no banco (dentro do container 'db' do mesmo grupo) - rode"
echo "de forma INTERATIVA (o -it evita problema de aspas do 'az container exec'"
echo "com comandos de uma linha), digite o psql na hora, ideal para o video:"
echo ""
echo "  az container exec --resource-group ${RESOURCE_GROUP} --name ${CONTAINER_GROUP} --container-name db --exec-command \"/bin/sh\""
echo "  # dentro do shell do container:"
echo "  psql -U ${DB_USER:-ollipet} -d ${DB_NAME:-ollipet}"
echo "  # dentro do psql:"
echo "  SELECT * FROM pet;"
